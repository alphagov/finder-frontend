...
RSpec.describe EntryPresenter do
  describe "#summary" do
    ...
    it "displays the truncated description" do
      expect(described_class.new(document, true).summary).to eq("This is the summary. And this is extra.")
    end

    it "returns nil" do
      expect(described_class.new(document, false).summary).to be_nil
    end
  end
  ...
  describe "#tag" do
    it "returns a tag" do
      ...
      expect(described_class.new(document, true).tag(atom_feed_builder)).to eq("tag:www.test.gov.uk,2019:/path/to/content")
    end
  end
  ...
  describe "#updated_at" do
    it "returns the public timestamp" do
      ...
      expect(described_class.new(document, true).updated_at).to eq("Thu Nov 29 14:33:20 2001")
    end
    ...
    it "returns the release timestamp" do
      ...
      expect(described_class.new(document, true).updated_at).to eq("Thu Nov 29 14:33:20 2010")
    end
  end
  ...
  describe "#feed_ended_id" do
    it "returns a feed_ended_id" do
      atom_feed_builder = ActionView::Helpers::AtomFeedHelper::AtomFeedBuilder.new(nil, nil, schema_date: 2019)
      expect(described_class.feed_ended_id(atom_feed_builder, "/path/to/content")).to eq("tag:www.test.gov.uk,2019:/path/to/content/feed-ended")
    end
  end
end
